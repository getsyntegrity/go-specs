...
// Environment variables that carry run coordination. Contract v1.2.6 §5 defines each one; they
...
// DefaultBaseDir is used when EnvReportDir is unset (contract v1.2.6 §5).
...
const (
	maxRunIDLen = 128
	// Token bounds in bytes, before hex encoding. The floor is normative (contract v1.2.6 §10):
...
// independently-launched package binaries would agree on (contract v1.2.6 F8).
//
...
// (contract v1.2.6 §10). Nothing may treat a matching token as an authorization decision.
...
// contract v1.2.6 §10 requires traversal segments rejected before the value reaches a path.
...
// of use rather than here (contract v1.2.6 §10).
//
...
//
// The token value is never placed in the returned error: contract v1.2.6 §5 forbids echoing it.
...
// do — making it both rare and extremely hard to diagnose (contract v1.2.6 §5).
//
...
// then compares in constant time (contract v1.2.6 §5).
//
