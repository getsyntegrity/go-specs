...
// explicitly, never partially trusted (contract v1.2.6 §10).
...
// deduplication and coverage arithmetic (contract v1.2.6 §9).
type ShardEnvelope struct {
	...
	// shards/ and have it merged into the module report (contract v1.2.6 §5, §10). The raw token
	// is never serialized.
