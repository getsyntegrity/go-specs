...
// may be reworded freely (contract v1.2.6 §5).
type ConfigErrorReason string

// The closed set from contract v1.2.6 §5. Anything not in this list is not a configuration error.
const (
...
// red build rather than one that just says "invalid". Contract v1.2.6 §5 makes that normative
...
// reporting is the silent failure this type exists to prevent (contract v1.2.6 §5, §8).
type ConfigError struct {
	...
	// EnvRunToken faults — contract v1.2.6 §5 forbids echoing the token, even a malformed one,
