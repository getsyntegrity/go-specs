...
// genuine *testing.T to prove a real Goexit ends only its group's subtest and therefore uses the subprocess
...
// inside a BeforeAll (runtime.Goexit) ends only that group's own subtest and does not abort
