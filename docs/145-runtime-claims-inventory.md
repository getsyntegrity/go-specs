...
| A3 | The `st_nlink == 2` inference depends on two invisible preconditions: the temp file is linked nowhere else, and nothing unlinks a published shard while the run is in flight. | §10 | **Pinned** — the first precondition rests on two distinct properties, each pinned separately: `TestTempFileLivesInTheDestinationDirectoryAndCarriesThePID` pins that `tempName` returns a bare, PID-bearing name (a name property only — it never calls `writeAndPublish`), and `TestWriteAndPublishOpensTheTempFileInsideTheDestinationDirectory` pins the effective temp-file location by observing, at `writeAndPublish`'s actual open call, that the path sits inside the destination directory (mutation-checked: building it outside that directory fails the test). The second precondition is pinned by `TestPublishNeverRemovesAPublishedFile`, which drives all four publish outcomes and asserts the final path is never the argument to `remove`. |
...
| A6 | Neither `rename` nor `link` is atomic across a filesystem boundary (`link` fails `EXDEV`), which is why the temp file must live inside the destination shard directory. | §5, §10 | **Pinned (structural)** — the same-filesystem precondition itself, not just the name shape: `TestWriteAndPublishOpensTheTempFileInsideTheDestinationDirectory` intercepts the path `writeAndPublish` actually opens and asserts it sits inside `dir`, mutation-checked by building the temp path outside `dir` (e.g. under `os.TempDir()`) and confirming the test fails. `TestTempFileLivesInTheDestinationDirectoryAndCarriesThePID` additionally pins that `tempName` returns a bare name with no directory component of its own, so it can only ever be joined into whatever directory it is given. Provoking a real `EXDEV` needs two mounts and is not portable in CI. |
...
| E2 | `umask` does not apply to a subsequent `Chmod`, which is why a directory this implementation creates must have its mode **verified after creation** rather than assumed from the requested `0700`. | §10 rule 3 | **Unpinned** — the claim as stated, that a *newly created* directory's mode is verified after the `Chmod` that sets it, cannot be distinguished by any achievable test: `umask` only clears bits, and the only bits it could clear from `0700` are the owner bits, which break `MkdirAll` before `Chmod` is ever reached. `TestCreatedDirectoriesGetModeSevenHundredRegardlessOfUmask` asserts the created mode is `0700`, but it passes under every umask that lets creation succeed at all, whether or not the post-`Chmod` verification step exists — so no umask value can make it fail on that verification specifically. A related but different invariant is genuinely pinned, and kept separate rather than credited to this claim: a *pre-existing* directory with a wider mode is refused rather than silently tightened, by `TestPreExistingRunDirectoryWithAWiderModeIsRefused`, which exercises `mkdirSecure`'s pre-existing-directory branch, not its newly-created/post-`Chmod` branch. |
...
- **E2** cannot be pinned as stated: the umask test its row demanded cannot be written, because
  `umask` can only clear bits that also break `MkdirAll` before the `Chmod` runs. The test cited
  against it exercises a different, genuinely pinned invariant instead, so the row is classified
  Unpinned rather than credited with a pin it does not have.
- **A3/A6** looked pinned by a test — `TestTempFileLivesInTheDestinationDirectoryAndCarriesThePID`
  — that names the right property but calls only `tempName()`, never `writeAndPublish` itself. A
  test that observes the real write boundary was missing; adding it is the one code change in this
  reconciliation.
...
- **Mutation-checked**, meaning the test was *observed* to fail when the behaviour is removed:
  A1/A2 (publish via `os.Rename`), A6 (the temp path built outside the destination directory at
  `writeAndPublish`'s write boundary), B1 (`Scan` rewritten as `os.LookupEnv`), D1 (`HashRunToken`
  over the hex text), E1 (`O_NOFOLLOW` dropped from the read site). A pin never seen to fail is a
  claim, not a pin.
- **Unpinned:** E2 — the newly-created-directory, post-`Chmod` verification this row claims cannot
  be distinguished by any achievable umask, so no test pins it. The row keeps this separate from the
  genuinely pinned, but different, pre-existing-directory-refused invariant.
- **Partly unpinnable, and said so rather than rounded up:** E1 at its three write sites, where
  `O_EXCL` makes the flag redundant and no test can distinguish it.
