...
| JSON | `report.RenderJSON` | Schema-versioned (`schemaVersion: "1"`; the `pending` totals field and per-case `"status": "pending"` are additive, so they don't bump it). Arrays are always arrays, never `null`. A consumer decoding into a struct with only a subset of fields is unaffected by new fields — see `report/render_json_test.go`'s `TestRenderJSONUnknownFieldsIgnorable`. |
