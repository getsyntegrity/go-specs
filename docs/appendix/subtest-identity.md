# Appendix · Subtest identity and generated candidate identity

> **Status:** preserved verbatim from the pre-regeneration `docs/EXECUTION_MODEL.md`.
> These sections document hard-won, still-accurate subtleties: how declared names map to Go subtest
> names, how the two sequential models diverge on hook frequency, how a filtered spec is reported,
> and how a narrowed `-run` perturbs adaptive path strategies. They are kept word-for-word because
> the reasoning, not just the conclusion, is the value.

Parent document: [03 · Execution model](../03-EXECUTION-MODEL.md).
...
> **Correction — no longer true.** This section describes behaviour that has since been fixed
> (#111). The runner now threads a `ran` flag out of the `t.Run` closure, because `t.Run`'s own
> boolean return is `true` even for a subtest the filter discarded. A filtered spec is reported with
> its own status, `Filtered`, distinct from both `Passed` and `Skipped`
> (`specs/runner.go:230`, `specs/runner.go:254`, `report/model.go:20`). The paragraph below is kept
> for the reasoning it records about why the naive `t.Run` return value cannot be used.
