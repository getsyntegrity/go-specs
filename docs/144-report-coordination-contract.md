...
### F5 — Timeout/panic bypasses any code after `m.Run()`
...
`-timeout` kill — and, by the same mechanism, an external `SIGKILL`/OOM-kill — terminates the
process via panic/force-exit without ever returning control to the caller's post-`m.Run()`
code. Any finalization logic living there (a `Flush`/shard-write call) simply does not run.
...
| Lock-file / counter that increments per package and triggers merge at N | Requires knowing N in advance and reliably decrementing on every exit path, including the F5 kill/timeout case where no code runs after `m.Run()`. A counter can't observe a process that never got to decrement it — indistinguishable from "still running." |
...
2. **`go test ./... -count=1 ...`** runs exactly as today. Every package that has already wired
   ...
   from its environment (F2: harmless to packages that don't). If set and valid, that package's
   `TestMain` additionally writes one shard file after its own `m.Run()` returns, via
...
4. **After `go test` returns**, the same script/CI step — only if it wants a merged module report
   ...
   ID, the shard base dir, and (if used) the path to the single combined `-coverprofile` file.
...
5. Finalize discovers the run's shard directory, accepts only complete (atomically-renamed) and
   valid shards, merges execution data deterministically, merges coverage by deduplicating
...
Environment variables (all optional; absence of `GO_SPECS_RUN_ID` ⇒ shard emission is a no-op,
preserving "disabled by default"):

| Variable | Set by | Meaning |
|---|---|---|
| `GO_SPECS_RUN_ID` | invoking script/CI, before `go test` | Opaque run identifier, `^[A-Za-z0-9_.-]{1,128}$`. Absent/invalid ⇒ no shard written, per-process #142 reporting still works unchanged. |
...
// ShardConfigFromEnv reads GO_SPECS_RUN_ID / GO_SPECS_REPORT_DIR. Enabled() is false, and every
// ShardWriter method is a no-op, when GO_SPECS_RUN_ID is unset or invalid — reporting stays
// disabled by default, matching #142/#143's existing invariant.
...
// ShardEnvelope is the on-disk shape of one shard file. Deliberately does not carry
// pre-aggregated coverage numbers (§9) — only a pointer to the coverage profile the run used,
// so the finalizer can locate and dedupe coverage exactly once.
...
type FinalizeOptions struct {
	...
	ExpectPackages []string // optional, e.g. from `go list ./...`, enables strict missing-shard checks
...
// reports. It lists only final (non-.tmp) shard names, rejects corrupt/duplicate/wrong-run/
// incompatible-version shards explicitly (never silently drops or silently picks one), merges
...
type FinalizeResult struct {
	...
	PackagesMissing []string        // in ExpectPackages but no valid shard found
	Rejected        []RejectedShard
}

type RejectedShard struct {
	Path   string
	Reason string // "corrupt" | "duplicate-package" | "schema-version-mismatch" | "wrong-run-id" | "stale"
}
```
...
| Tests fail, reporting succeeds | unchanged (1) | 0 | Per-process shard write still happens (`TestMain`'s post-`m.Run()` code runs on ordinary failure, only not on panic/timeout — F5) so the failure is fully represented in the merged report. |
...
| Process interruption/timeout (F5) | whatever `go test`/the OS already reports for a kill/timeout | Finalize reports that package under `PackagesMissing` or `Rejected`, not silently | The killed package's shard was never written; finalize's job is to make that fact loud, not to guess. |
...
- **#145 (shard producers) must not compute or embed per-package coverage numbers.** A
  ...
  zero-value) plus, optionally, a pointer to the coverage profile path used for the run.
...
| 4 | Existing Go flags and package discovery retain normal behavior | No new required flags at all (§1 F1, §2); activation is env-var only, `go test`'s own flag/discovery behavior is untouched |
...
| 6 | Cache behavior explicit and tested in the later integration slice | §6/§1 F4: `-count=1` required, documented as a stated tradeoff; missing-shard-on-cache-hit behavior specified for #146 to test |
...
- **#145**: scope should be tightened to explicitly exclude per-package coverage computation
  (§9) — shards carry execution data only, plus an optional coverage-profile path pointer — and
...
- **#146**: scope should explicitly add (a) a block-deduplicating coverage merge keyed by
  ...
  and (b) explicit, tested handling of "missing shard" as its own condition (cache skip, timeout
  kill, or genuine failure to launch — §6/§8), never silently treated as "package had nothing to
  report."
...
- **Strict vs. lenient missing-shard default**: recommend strict-by-default (fail the finalize
  step if `ExpectPackages` includes a package with no valid shard) with an explicit opt-out for
  local/dev use — final default is a #146/product call.
