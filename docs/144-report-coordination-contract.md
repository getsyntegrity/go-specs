...
|---|---|---|
| `GO_SPECS_RUN_ID` | invoking script/CI, before `go test` | Opaque run identifier, `^[A-Za-z0-9_.-]{1,128}# REPORT-002A: Native multi-package reporting coordination contract
...
| Variable | Set by | Meaning |
|---|---|---|
. Absent ⇒ coordination disabled. Present but invalid ⇒ configuration error; the process must fail loudly and must not silently disable shard emission. |
